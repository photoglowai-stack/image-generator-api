// /api/v1-preview.mjs — Commercial Photoreal Preview (V6.1)
// - JSON only + CORS (Figma OK)
// - GET: health/debug
// - POST: Pollinations (POST -> fallback GET) -> upload Supabase -> URL Supabase
// - Preview rapide (fast=true par défaut); HQ possible (fast=false)
// - Backoff 429/5xx + timeouts budgétés Vercel
// - Cache "preview_cache" (best-effort); clés d’objets courtes & stables
...
const POL_TIMEOUT_MS   = Math.max(4000, Math.min(TIME_BUDGET_MS - 1500, 18000));      // fenêtre Pollinations
...
// Choix contrôlés (prompt builder compact)
...
async function fetchPollinationsBinary(params) {
  const RETRIES = 2;               // total 1 + 2 = 3 tentatives
  let lastErr;
  for (let i = 0; i <= RETRIES; i++) {
    try {
      return await pollinationsAttempt(params);
    } catch (e) {
      lastErr = e;
      const msg = String(e?.message || e);
      const retriable = /pollinations_http_(429|5\d{2})|timeout|fetch failed/i.test(msg);
      if (!retriable || i === RETRIES) break;
      const backoff = 400 * Math.pow(2, i);  // 400ms, 800ms
      await sleep(backoff);
    }
  }
...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    return res.status(200).json(dbg
      ? { ok: true, endpoint: "/v1/preview", has_supabase_url: hasUrl, has_service_role: hasSrv, bucket: BUCKET, output_public: OUTPUT_PUBLIC, poll_token: Boolean(POL_TOKEN) }
  ...
  try {
    ...
    if (!ok(body?.prompt)) body.prompt = buildPrompt(normalized);

    // Rendu
    const fast = toBool(body?.fast ?? true);                   // fast par défaut
    const ratio = normalized.ratio;
    const [W, H] = (fast ? SIZE_FAST : SIZE_HQ)[ratio] || (fast ? [576, 576] : [896, 896]);
    const seed = deriveSeed(body?.seed, normalized, `${normalized.framing}|${normalized.lighting}|${normalized.lens}`);
    const safe = toBool(body?.safe ?? true) ? "true" : "false"; // Safe par défaut
    const prompt = String(body.prompt);

    // Cache key (DB) — non utilisée pour l’objet Storage
    const cacheKey = `${STYLE_VERSION}${fast ? "|fast" : ""}|${prompt}|seed:${seed}|${W}x${H}`;

    // Lookup cache (best-effort)
    try {
      const cached = await sb.from("preview_cache").select("image_url,hits").eq("key", cacheKey).maybeSingle();
      if (cached?.data?.image_url) {
        // incrément hits (non-bloquant)
        sb.from("preview_cache").update({ hits: (cached.data.hits || 0) + 1 }).eq("key", cacheKey).then(()=>{},()=>{});
        return res.status(200).json({ ok: true, image_url: cached.data.image_url, provider: "cache", seed, key: cacheKey, fast: !!fast });
      }
    } catch (e) {
      console.warn("[cache.lookup] non-blocking:", e);
    }
    ...
    // Upload Supabase — clés courtes: v2[-fast]-s{seed}-{W}x{H}-{hash}.{ext}
    const d = new Date();
    ...
    // Insert cache (best-effort)
    sb.from("preview_cache").insert({ key: cacheKey, image_url: imageUrl }).then(()=>{}, (e)=>console.warn("[cache.insert] non-blocking:", e));
